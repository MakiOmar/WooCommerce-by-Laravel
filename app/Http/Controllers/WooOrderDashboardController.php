...
class WooOrderDashboardController extends Controller
{
    ...
    {
        $filters = [
            'per_page' => config('woo-order-dashboard.pagination.per_page', 15),
            'page' => request('page', 1)
        ];
        
        $cacheKey = 'woo_orders_' . md5(json_encode($filters));
        
        $orders = Cache::remember($cacheKey, config('woo-order-dashboard.cache.ttl.orders', 300), function () use ($filters) {
    ...
    public function show($id)
    {
        ...
        $order = Cache::remember($cacheKey, config('woo-order-dashboard.cache.ttl.order', 300), function () use ($id) {
    ...
    protected function validateFilters(Request $request)
    {
        return $request->validate([
            ...
            'per_page' => 'nullable|integer|min:1|max:' . config('woo-order-dashboard.pagination.max_per_page', 100),
        ]);
    }
} 